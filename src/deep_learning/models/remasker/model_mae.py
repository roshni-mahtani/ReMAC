...
from utils.utils_remasker import MaskEmbed, get_1d_sincos_pos_embed, ActiveEmbed
eps = 1e-6
...
class MaskedAutoencoder(nn.Module):
    ...
    def random_masking(self, x, m, mask_ratio):
        ...
        #print(f"L (longitud input): {L}")
        if self.training:
            len_keep = int(L * (1 - mask_ratio))
        else:
            len_keep = int(torch.min(torch.sum(m, dim=1)))
        #print(f"Mask ratio: {mask_ratio}")
        #print(f"len_keep (tokens visibles): {len_keep}")
    ...
    def forward_decoder(self, x, ids_restore):
        ...
        decoder_output = x  # ← salida del decoder antes de proyección
    ...
    def forward_loss(self, data, pred, mask, nask):
        ...
        loss_mask = (loss * mask).sum() / mask.sum() if mask.sum() > 0 else 0
        loss_nask = (loss * nask).sum() / nask.sum() if nask.sum() > 0 else 0

        return loss_mask + loss_nask


    def forward(self, data, miss_idx, mask_ratio=0.5):
        
        latent, mask, nask, ids_restore = self.forward_encoder(data, miss_idx, mask_ratio)
        pred, decoder_output = self.forward_decoder(latent, ids_restore)
